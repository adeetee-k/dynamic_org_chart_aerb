...
export class OrgDataService {
    ...
    /** Get Section Heads reporting to a given manager */
    getSectionHeads(managerId: string): Employee[] {
        return this.sortEmployees(this.employees.filter(e => e.managerId === managerId && e.designation === 'Section Head'));
    }

    /** Get all non-PA descendants under a manager, excluding a designation set */
    getDeeperDescendants(managerId: string, excludeDesignations: Set<string> = new Set(['Section Head'])): Employee[] {
        const result: Employee[] = [];
        const stack: string[] = [managerId];
        while (stack.length > 0) {
            const id = stack.pop() as string;
            const children = this.employees.filter(e => e.managerId === id && e.designation !== 'PA');
            for (const child of children) {
                if (!excludeDesignations.has(child.designation)) {
                    result.push(child);
                }
                stack.push(child.id);
            }
